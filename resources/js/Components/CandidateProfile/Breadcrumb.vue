        ...
        <ol class="breadcrumb p-2">
            ...
            >
                <template v-if="candidate.location">
                    <a :href="route('candidate-profiles.index')" class="text-decoration-none" :class="{ disabled: isLast(item) }"> {{ location.name }} </a>
                </template>
                <template v-else>
                    <a :href="route('candidate-profiles.index')" class="text-decoration-none" :class="{ disabled: isLast(item) }"> National </a>
                </template>
            </li>
            <li class="breadcrumb-item" aria-current="page">
                <template v-if="candidate.running_position_id='8'">
                    <a href="#" class="text-decoration-none"> Presidential Candidates </a>
                </template>
